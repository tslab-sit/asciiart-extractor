...
public class GRLSAttributesExtractor {
	public static void main(String[] args) {
		GRLSDataSetPrinter dataSetPrinter = null;
		...
		if (format.equals("arff") || format.equals("ARFF")) {
			dataSetPrinter = new GRLSARFFDataSetPrinter();
		} else if (format.equals("svm") || format.equals("SVM")) {
			dataSetPrinter = new GRLSSVMDataSetPrinter();
		} else {
		...
		try {
			...
			GRLSAttributesFactory attrFactory = new GRLSAttributesFactory();
			for(; argIndex<args.length; argIndex++) {
				...
				for(int windowSize=1; windowSize<=maxWindowSize; windowSize++){
					...
					while((window = tr.getNextWindow()) != null) {
						GRLSAttributes attr = attrFactory.create(window);
